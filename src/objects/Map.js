...
class Map {
  ...
  renderMap() {
    ...
    var foodStorageGroup = this.game.add.group();
    var incubatorGroup = this.game.add.group();
    var pavilionGroup = this.game.add.group();
    var cageGroup = this.game.add.group();
    var slaughterhouseGroup = this.game.add.group();
    var carcassStorageGroup = this.game.add.group();
    var furStorageGroup = this.game.add.group();
    var tile, offset;

    // draw background
    var background = this.game.add.sprite(0, 0, 'background', 0, groundGroup);
    ...
    for(var i = 0; i < Simulator.map.length ; i++) {
      ...
      for(var j = 0; j < Simulator.map[i].length; j++) {
        ...
        if(index == 1) {
          tile = new FoodStorage(this.game, x, y, 'food_storage', 0, foodStorageGroup);
          Farm.foodStorage = tile;
          tile.anchor.set(0.5);
        }

        if(index == 2) {
          tile = new Incubator(this.game, x, y, 'incubator', 0, incubatorGroup);
        ...
        if(index >= 10 && index <= 31) {
          ...
          tile = new Cage(this.game, x, y, enabled ? 'cage_double_full' : 'cage_double_empty', 0, cageGroup, enabled, pavilion);
          Farm.cages.push(tile);
          tile.anchor.set(0.65, 0.4);
        }

        if(index == 3) {
          var pavilion_back = this.game.add.sprite(x, y, 'pavilion_back', 0, pavilionGroup);
          pavilion_back.anchor.set(0.25, 0.91);

          var pavilion_front = new Pavilion(this.game, x, y, 'pavilion_front', 0, pavilionGroup);
        ...
        if(index == 50) {
          var slaughterhouse_back = this.game.add.sprite(x, y, 'slaughterhouse_back', 0, slaughterhouseGroup);
          slaughterhouse_back.anchor.set(0, 0);
        }

        if(index == 5) {
          var slaughterhouse_front = new Slaughterhouse(this.game, x, y, 'slaughterhouse_front', 0, slaughterhouseGroup);
          Farm.slaughterhouse = slaughterhouse_front;
          slaughterhouse_front.anchor.set(1);
        }

        if(index == 6) {
          tile = new KillingStation(this.game, x, y, 'killing_robot', 0, slaughterhouseGroup);
          Farm.killingStations.push(tile);
          tile.anchor.set(0.5);
        }

        if(index == 7) {
          tile = new SkinningStation(this.game, x, y, 'skinning_robot', 0, slaughterhouseGroup);
          Farm.skinningStations.push(tile);
          tile.anchor.set(0.5);
        }

        if(index == 8) {
          var furStorage = new FurStorage(this.game, x, y, 'storage', 0, furStorageGroup);
          Farm.furStorage = furStorage;
          furStorage.anchor.set(0.5);
        }

        if(index == 81) {
          tile = this.game.add.sprite(x, y, 'furs', 0, furStorageGroup);
          tile.anchor.set(0.5);
        }

        if(index == 9) {
          var carcassStorage = new CarcassStorage(this.game, x, y, 'carcass', 0, carcassStorageGroup);
